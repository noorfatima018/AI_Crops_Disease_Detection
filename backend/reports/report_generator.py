...
def generate_single_report(prediction_id, output_path="single_report.pdf"):
    """
    Generates a PDF report for a single specific prediction.
    """
    from database.mongo import get_prediction_by_id
    
    p = get_prediction_by_id(prediction_id)
    if not p:
        return None
    ...
    elements.append(Paragraph("FASALAI — INDIVIDUAL DIAGNOSTIC REPORT", title_style))
    elements.append(Paragraph(f"Report ID: {prediction_id}", styles["Normal"]))
    ...
    details = [
        ...
        ["Model Used:", p.get("used_model", "Unknown")],
        ["Treatments:", treatments],
        ["Date/Time:", p.get("timestamp", "Unknown")]
    ]
    ...
    elements.append(details_table)
    elements.append(Spacer(1, 20))
    elements.append(Paragraph("<b>Disclaimer:</b> This report is generated by AI and should be verified by a local agricultural expert.", styles["Italic"]))
